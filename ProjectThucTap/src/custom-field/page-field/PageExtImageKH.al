pageextension 50150 PageExtImageKH extends PageKhachHang
{
    actions
    {
        addafter("View Detailss")
        {
    ...
    begin
        ...
        foreach EntryListKey in EntryList do begin
            ...
            Clear(Cust.hinhanh);
            Cust.hinhanh.ImportStream(EntryInStream, FileName);
            Cust.Modify(true);
            FileCount += 1;
            // gán hình ảnh cho hình ảnh blob
            Clear(Cust.hinhanhblob);
            Cust.hinhanhblob.CreateOutStream(EntryOutStream);
            CopyStream(EntryOutStream, EntryInStream);
            Cust.Modify();
            CurrPage.Update();
